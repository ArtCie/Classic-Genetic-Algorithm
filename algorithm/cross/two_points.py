    ...
    def evaluate(self, population_repository: PopulationRepository) -> None:
        ...
        while len(new_pop) < population_repository.population_size:
            ...
            if np.random.choice([0, 1], p=[1 - self.PROBABILITY, self.PROBABILITY]):
                cross_point = np.random.choice(range(1,population_repository.chromosome_length * 2 - 1),2)
                if cross_point[0] == cross_point[1]:
                    pass
                if cross_point[1] < cross_point[0]:
                ...
                new_individual_1 = merged_individual_1[:cross_point[0]] + merged_individual_2[cross_point[0]:cross_point[1]] + merged_individual_1[cross_point[1]:]
                new_individual_2 = merged_individual_2[:cross_point[0]] + merged_individual_1[cross_point[0]:cross_point[1]] + merged_individual_2[cross_point[1]:]
